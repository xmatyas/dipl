...
def create_index_map(path : str = "../input/emnist-bymerge-mapping.txt"):
    validate_path(path)
    index_map = {}
    with open(path) as f:
        for line in f:
            (key, val) = line.split()
            index_map[chr(int(val))] = int(key)
    return index_map
...
def filter_images(images : np.array, labels : np.array, class_map: str, filter_labels : str, reverse: bool = False) :
    ...
    # Creates a map of the labels to their indexes
    index_map = create_index_map()
    # List containing the indexes of the labels to be filtered from the labels array
    ...
    if reverse:
        ...
        label_index_list = [ index_map[key] for key in class_map if key not in filter_labels]
        # TODO simplify this so it removes this before creating the filtered_labels
        filtered_labels = np.delete(filtered_labels, label_index_list, axis=1)
    else:
        ...
        filtered_images = np.array(filtered_images)
        filtered_labels = np.array(filtered_labels)
        label_index_list = [ index_map[key] for key in filter_labels]
        # TODO simplify this so it removes this before creating the filtered_labels
        filtered_labels = np.delete(filtered_labels, label_index_list, axis=1)

    
    # Remove specific indexes from labels
    #label_indexes = [get_class_number_from_categorized_label(label) for label in filtered_labels]
    #filtered_labels = np.delete(filtered_labels, label_indexes)
...
#train_images, train_labels = filter_images(train_images, train_labels, "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabdefghnqrt","QO0", reverse = True)
#test_images, test_labels = filter_images(test_images, test_labels, "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabdefghnqrt","QO0", reverse = True)
#number_of_classes = number_of_classes - len("UV") # remove the filtered labels
#number_of_classes = len("QO") # filtered labels
...
def train_model(model: Sequential, train_images: np.array, train_labels: np.array, test_images: np.array, test_labels: np.array, epochs: int = 10):
    mc = ModelCheckpoint('emnist_model.h5', monitor='val_categorical_crossentropy', mode='max', verbose=1, save_best_only=True)
...
model = create_model(number_of_classes, 2, 32, (256,64))
print(model.summary())
model = train_model(model, train_images, train_labels, test_images, test_labels)
model.save(MODEL_FOLDER + 'emnist_model')
conf_matrix = get_confusion_matrix(model, test_images, test_labels)
print(conf_matrix)